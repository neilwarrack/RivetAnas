...
public:
  ...
  /// Typedef for a valaray of parameters to the function to be
  /// minimised.
  typedef std::valarray<double> Params;
  ...
  MendelMin(const FuncT & fin, const RndT & rndin, unsigned int ndim,
            unsigned int npop = 20, unsigned int ngen = 20,
            double margin = 0.1)
    : _f(&fin), _rnd(&rndin), _NDim(ndim), _margin(margin),
      _pop(npop), _fit(npop, -1.0), showTrace(false) {}

  /// Supply a best gues for the fittest parameter point to help
  /// things along.
  ...
  /// SImple wrapper arounf the function to be minimised.
